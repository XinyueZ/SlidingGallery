 ...
 */
public class LLImageDownloader
{
    ...
    /**
     * Download the specified image from the Internet and binds it to the provided ImageView. The binding is immediate if the image is found in the cache and will be done asynchronously otherwise. A null bitmap will be associated to the ImageView if an
     * error occurs.
     * 
     * @param url
    ...
    Bitmap downloadBitmap( String path )
    {
        Bitmap d = LLImageCache.getSendungenImage( path );
        if( d == null )
        {
            try
            {
                ...
                if( is != null )
                {
                    ...
                    is.close();
                    LLImageCache.setSendungImage( path, d );
                    return d;
                }
            }
            catch( IOException e )
    ...
    private static final int                                              DELAY_BEFORE_PURGE  = 1000;                                   // in milliseconds
